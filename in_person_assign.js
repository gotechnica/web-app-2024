...
let startTime = new Date(2024, 9, 26, 12, 0); // time that expo starts, will start assigning demos from here (month is 0-based)
...
const linkDict = {
  ...
  "Cheshire Classroom Creation: Best Education Hack":
  ...
  "Queen of Hearts Game Changer: Best Gamification Hack":
    "https://app.gather.town/app/2x7GW9NhurfCG9pf/Technica%202023-2024?spawnToken=UHb47IZnR72t_VaKFK0H",
  "WonderBridge Connecting All Users: Best Accessibility Solution":
...
csv()
  ...
  .then(async (csvResult) => {
    csvResult.forEach(
      ...
      (item) => {
        ...
        if (
          ...
          // idk why drafts show up in this csv, but ignoring them is good
          let isVirtual = item["Are You Being Judged In Person Or Virtually?"] !== "in-person"; // is this team presenting virtually or not?
          ...
          for (let i = 0; i < prizes.length; i++) {
            // iterate over all prize categories for this team (comma separated)
            ...
            if (!prizeString.startsWith("Best")) {
              ...
              if (!linkDict[prizes[i].trim()]) {
                console.error(
                  "Oh no! I didn't find a location for \"" +
                    prizes[i].trim() +
                    '"! check linkDict.'
                );
                exit();
            ...
            if (!timeReference[prizeName]) {
              // prize category hasn't been seen yet, initialize JSON object
            ...
            if (demoTime > endTime) {
              //keep track of latest demo time
              endTime = new Date(demoTime.getTime() + lengthOfMeeting * 60000);
            }
            let projectTitle = [item["Project Title"]];
            if (projectTitle.toString().includes(",")) {
              //replace any comments in project title, can mess up csv
            ...
            timeReference[prizeName][demoTime] = timeResult
              ? timeResult + 1
              : 1; // update the dictionary entry, add a mapping for this time slot
            ...
            finalCSV.push([
              ...
              sponsName,
              isVirtual
                ? linkDict[prizes[i].trim()]
                : getGroupTable(projectTitle),
            ]);
          }
    ...
    console.log("physical taken times: ", physicalTakenTimes);
    console.log("virtual taken times: ", virtualTakenTimes);
    console.log("physical locations: ", physicalLocations);

    let output =
      "team_name,start_time,end_time,prize_category,sponsor_name,location\n"; // define headings
    finalCSV.forEach((row) => {
      // creating csv format from array of arrays
      output += `${row.join(",")}\n`;
    });
    // https://www.w3schools.com/nodejs/nodejs_filesystem.asp
    fs.writeFile("test_data_schedule.csv", output, (err) => {
      if (err) throw err;
      console.log(
